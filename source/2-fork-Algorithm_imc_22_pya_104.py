...
PARAMS = {
    Product.TOMATOES: {
        "take_width": 6,   # spread is 13, so take only clear mispricing
        "clear_width": 4,
        "edge": 6.5,
        "adverse_volume": 5,
...
class Trader:
    ...
    def market_make(
        ...
        skew=params.get("inventory_accel",0) * position

        if position > 0:
            ask-= skew   #   push quotes asymmetrically. done
        else:
            bid += abs(skew)
    ...
    def emeralds_fair_value(self, order_depth: OrderDepth, traderObject) -> float:
        ...
        # -----------------------------
            filtered_ask = [
                ...
                if abs(order_depth.sell_orders[price]) >= params["adverse_volume"]
            ]

            filtered_bid = [
                price for price in order_depth.buy_orders
                if abs(order_depth.buy_orders[price]) >= params["adverse_volume"]
            ]
        ...
        # -----------------------------
            if mm_ask is None or mm_bid is None:
                if traderObject.get("emeralds_last_price") is None:
                    mmmid_price = (best_ask + best_bid) / 2
                else:
                    mmmid_price = traderObject["emeralds_last_price"]
            else:
                mmmid_price = (mm_ask + mm_bid) / 2
        # -----------------------------
        # 🔥 CORE: MICROPRICE
        # -----------------------------
            ...
            fair = (
                (1 - params["microprice_weight"]) * mmmid_price
                + params["microprice_weight"] * microprice
            )

            traderObject["emeralds_last_price"] = mmmid_price
...
# =========================
        if abs(zscore) > trigger_z:
            ...
            if zscore > 0 and position > -position_limit:
                safe_order(best_bid - 1, -quantity)    # selling BELOW best bid — worst possible price <----------------

            elif zscore < 0 and position < position_limit:
                safe_order(best_ask + 1, quantity)   # buying ABOVE best ask — worst possible price  <----------------
            return orders 
